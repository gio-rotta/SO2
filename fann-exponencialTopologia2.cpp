...
fann_train_data* createData()
{
	...
	trainingSet = fann_create_train ( DATA_NUMBER_ROWS, 2, 1 );

	for ( int i = 0; i<DATA_NUMBER_ROWS; i++ ) {
        float x = rand()%10;
        float z = rand()%10;
		trainingSet->input[i][0] = x;
		trainingSet->input[i][1] = z;
		trainingSet->output[i][0] = f ( x, z );
	}
	return trainingSet;
}
...
void testData(fann* ann){
    ...
    for ( int i = -5; i<15; i++ ) {
        for (int j=0;j<15;j++){
            ...
            float error = meuabs( f(x[0], x[1])- *o );
            std::cout<< "f("<<x[0]<<", "<<x[1]<<")="<<f(x[0], x[1])<<"  received:"<<*o<< "  error:"<< error <<std::endl;
        }
...
int main()
{
   // srand(time(0));
    ...
    testData(ann);
    fann_save(ann,"fann-exponencialTopologia2.net" );
